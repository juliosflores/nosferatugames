...
Regras: identifique o título exato pela capa.`;
    ...
    try {
      ...
      if (resp.ok && respData.choices) {
        ...
        // RETORNAR O ERRO REAL PARA O NAVEGADOR VER
        return res.status(resp.status).json({ 
          error: 'Erro no Aurora', 
          details: respData 
        });
      }
    } catch (e) {
      return res.status(500).json({ error: 'Falha na conexão VPS', details: e.message });
    }
  } catch (e) {
