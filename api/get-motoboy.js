...
export default async function handler(req, res) {
  ...
  // Coordenadas da Nosferatu Games (Goethe, 71) - [Longitude, Latitude]
  const ORIGEM = [-51.217743, -30.034608]; 

  try {
    // 1. Geocodificar o CEP de destino
    ...
    const DESTINO = geoData.features[0].geometry.coordinates; // [Long, Lat]

    // 2. Calcular a distância de rota (Direções)
    ...
    // 3. Aplicar a fórmula: Taxa Mínima R$ 10 + (KM * 1.90)
    let precoFrete = 10 + (distanciaKM * 1.90);
